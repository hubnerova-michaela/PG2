...
const std::vector<std::string> houseModels = { "house", "bambo_house", "cyprys_house", "building" };

void spawnNewHousesCpp(CupcakeGame& game, PhysicsSystem& physics, float zPosition) {
	// --- Generation Parameters ---
	static std::mt19937 rng(std::random_device{}()); // Random number generator
	static std::uniform_real_distribution<float> dist(0.0f, 1.0f);
	const float houseSideOffset = 10.0f;
	const float emptyPlotProbability = 0.2f; // 20% chance of an empty plot

	// --- Left Side of the Road ---
	if (dist(rng) > emptyPlotProbability) {
		House house;
		house.id = game.getGameState().nextHouseId++;
		house.modelName = houseModels[rng() % houseModels.size()]; // Pick a random model
		house.position = glm::vec3(-houseSideOffset, 0.0f, zPosition);
...
int main()
{
	try
	{
...

		{
			float z = -15.0f;
			for (int i = 0; i < 20; ++i) {
				// Call the C++ generation function we created earlier
				spawnNewHousesCpp(*cupcakeGame, *physicsSystem, z);
		...
		while (!glfwWindowShouldClose(window))
		{
			...
			if (camera && cupcakeGame && cupcakeGame->getGameState().active)
			{
...

				{
					...
					// If the player is getting close to the edge of the generated world...
					if (camera->Position.z - farthestZ < 200.0f) {
						// ...spawn a new row of houses further down the road.
						float newZ = farthestZ - cupcakeGame->getGameState().houseSpacing;
						spawnNewHousesCpp(*cupcakeGame, *physicsSystem, newZ);
					}
				}
			...
			// List of available house models for C++ generation
			const std::vector<std::string> houseModels = { "house", "bambo_house", "cyprys_house", "building" };

			// C++ function to spawn a new row of houses

			for (const auto& h : cupcakeGame->getGameState().houses) {
				// Get the specific model name for this house (e.g., "bambo_house")
				std::string modelName = h.modelName;
				if (modelName.empty()) {
					modelName = "house"; // Use "house" as a default
				}

				// Find the correct model in our scene
				if (scene.find(modelName) != scene.end()) {
					glm::vec3 pos = h.position;
					glm::vec3 rot(0.0f); // No rotation for now
					glm::vec3 scl(1.0f); // Default scale

					// Set a specific, VISIBLE scale for each house type
					if (modelName == "bambo_house") {
						scl = glm::vec3(2.0f); // Larger scale
					}
					else if (modelName == "cyprys_house") {
						scl = glm::vec3(2.5f); // Larger scale
					}
					else if (modelName == "building") {
						scl = glm::vec3(1.5f); // Larger scale
					}
					else if (modelName == "house") {
						scl = glm::vec3(0.1f); // This model is huge, so it needs a smaller scale
					}

					// Draw the correct house model at the correct position
